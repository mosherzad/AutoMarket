...
const MobileNavigation = async () => {
  ...
  return (
    <nav className="container mx-auto max-w-7xl fixed bottom-0 left-0 px-3 py-1 right-0 bg-white border-t border-gray-200 lg:hidden z-50">
      <div className="flex items-center justify-between px-6">
        ...
        >
          <MdExplore size={30} />
          <span className="text-[12px]">{t("explore")}</span>
        </Link>
        {token && (
          <div className="">
            <Link
            ...
            >
              <MdPostAdd size={30} />
              <span className="text-[12px]">{t("addCar")}</span>
            </Link>
          </div>
        )}
        {userPayload?.isAdmin && (
          ...
          >
            <MdDashboard size={30} />
            <span className="text-[12px]">{t("dashboard")}</span>
          </Link>
        )}
      </div>
    </nav>
